...
from keras.layers.fake_approx_convolutional import FakeApproxConv2D as Conv2D

MTAB_FILE_NAME = sys.argv[1]
mtab_file = f"../../axmul_8x8/{MTAB_FILE_NAME}"
...
def mnist_model():
    ...
    model_layers = [ tf.keras.layers.Input(shape=(28,28,1), name="model/input"),
                     Conv2D(16, (3, 3), padding="same", activation="relu", kernel_initializer='he_normal',
                            name="clf/c1", mul_map_file=mtab_file),
                     Conv2D(16, (3, 3), padding="same", activation="relu", kernel_initializer='he_normal',
                            name="clf/c2", mul_map_file=mtab_file),
                     tf.keras.layers.MaxPooling2D(pool_size=(2, 2), name="clf/p1"),
                     Conv2D(16, (3, 3), padding="same", activation="relu", kernel_initializer='he_normal',
                            name="clf/c3", mul_map_file=mtab_file),
                     Conv2D(16, (3, 3), padding="same", activation="relu", kernel_initializer='he_normal',
                            name="clf/c4", mul_map_file=mtab_file),
...
def arg_parser(parser):
    ...
    parser.add_argument("--path" , dest ="path", type=str, default='./examples/tf_model.weight.h5', help="path of tf.keras model's wieghts")
